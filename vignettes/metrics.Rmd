...
scc_by_file(fixtures)[, c("filename", "lines", "code",
...
`dryness = uloc / lines` (matching `scc`'s `DRYness %` formula in its tabular output, applied per record instead of project-wide). Values close to 1 mean a file or language is mostly unique non-blank content; values closer to 0 mean heavy duplication, comments, or blanks.

Note the `dryness = TRUE` flag here is unrelated to the column — it asks `scc` to print an extra `DRYness %` line in its tabular console output (which we never see because `glockr` always requests JSON). The `dryness` column is populated whenever `uloc = TRUE`.

```{r dryness}
scc(rlang_path)[, c("language", "lines", "code", "uloc", "dryness")] |>
  gt::gt() |>
  gt::tab_header(
